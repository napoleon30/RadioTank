...
int MyFFmpeg::init(char *rtspURL) {
	...
	//2   这个api读出文件的头部信息，并做demux解复用             连接RTSP服务器，打开视频流端口
	...
	if (avformat_find_stream_info(pFormatCtx, &pOptions) < 0) {
		avformat_close_input(&pFormatCtx);			//如果没有找到流信息，就关闭
	...
	//5    找到对应的解码器，ffmpeg3.2以后参数应使用AV_CODEC_ID_H264
	...
	if (startDecod) {
		...
		if (packet.stream_index == videoStream) {
			...
			while (pktSize > 0) {
				...
				if (pFrame->pict_type == 1) {  //pFrame为I帧
					startRe = true;
				}
				...
				if (gotframe) {
					...
					pixel_queue_put(&pixelQueue, pixelPkt);		//转换成yuv的像素点放入  像素队列pixelQueue中
				}
				av_frame_unref(pFrame); //释放pFrame
...
int MyFFmpeg::encoder(char *filename) {
	...
	ret = 0;
	av_write_trailer(o_fmt_ctx);    //写输出流尾
